...
class Telegram
{
     ...
     * \param $endpointer the url endpoint telegram & google
     * \param $proxy array with the proxy configuration (url, port, type, auth)
     * \return an instance of the class.
     */
    public function __construct(string $bot_token, string $endpointer = 'telegram', array $proxy = [])
    {
    ...
    {
        $newContent = $content;
        if ($this->endpointer === 'telegram') {
            $url = 'https://api.telegram.org/bot' . $this->bot_token . '/' . $api;
        } else {
            $url = 'https://script.google.com/macros/s/AKfycbxrsve8vS7YMb5aCzwtPeD4e8JUPAhih9rcVmlEkgIMkgGujgBjDijRw2fLu5JyVvSA/exec';
            $newContent = array(
        ...
        if ($post) {
            $reply = $this->sendAPIRequest($url, $newContent);
        } else {
        ...
        if ($this->endpointer === 'telegram') {
            ...
            return (gettype($reply) === "array") ? $reply : json_decode($reply, true);
        }
    }
